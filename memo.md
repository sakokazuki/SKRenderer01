...
### 4/10
分散シャドウマップ(variance shadow map)というソフトシャドウの手法を知ったので実装してみた。
確率統計用語が全然わからなくそこから学習していた。平均と分散、チェビシェフの不等式。
一度理解してみるとこんな方法あったんだという感じで実装はYoutubeで解説してくれる動画が
みつけてシェーダー部分はカンニングした。https://www.youtube.com/watch?v=LGFDifcbsoQ
何をしているかは一通り理解したのでまぁいいんじゃないだろうか。
影がすごくきれいになったので満足度高い。